...
export const categoriesApi = {
  ...
  getAll: async (params?: { skip?: number; take?: number }): Promise<Category[]> => {
    ...
    const data = await api.get<any[]>(`/categories${queryParams}`)
  ...
  getWithPath: async (id: string): Promise<CategoryWithPath> => {
    const data = await api.get<any>(`/categories/${id}/path`)
  ...
  create: async (data: CreateCategory): Promise<Category> => {
    ...
    const response = await api.post<any>('/categories', validatedData)
  ...
  update: async (id: string, data: UpdateCategory): Promise<Category> => {
    ...
    const response = await api.put<any>(`/categories/${id}`, validatedData)
  ...
  count: async (): Promise<{ count: number }> => {
    const data = await api.get<any>('/categories/count')
