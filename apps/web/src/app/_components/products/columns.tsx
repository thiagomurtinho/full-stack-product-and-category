...
export const createColumns = ({ onEdit, onDelete, onView }: ColumnsProps): ColumnDef<ProductWithCategories>[] => [
  {
    ...
    cell: ({ row }) => {
      ...
      const categoryPath = Array.isArray(product.categoryPaths) && product.categoryPaths.length > 0 
        ? product.categoryPaths.sort((a, b) => b.length - a.length)[0] 
        : 'products'
      ...
      return (
        <button
          ...
          title={`View details for ${String(product.name || 'Unknown Product')}`}
        >
    ...
    cell: ({ row }) => {
      ...
      return (
        <div className="text-sm text-muted-foreground">
          {safeCategoryIds.length > 0 ? `${safeCategoryIds.length} category(ies)` : "No categories"}
        </div>
      )
    },
  },
