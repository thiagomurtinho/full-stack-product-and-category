...
export function ProductsTable({ selectedCategoryIds = [], isCategoryFilterPending }: ProductsTableProps) {
  ...
  const handleView = (product: ProductWithCategories) => {
    ...
    const categoryPath = Array.isArray(product.categoryPaths) && product.categoryPaths.length > 0 
      ? product.categoryPaths[0] 
      : 'products'
    ...
    return (
      <div className="space-y-4 w-full">
        ...
        <div className="rounded-md border p-4 bg-red-50">
          <p className="text-red-600">Error loading products: {String(error)}</p>
        </div>
      </div>
    )
  }
      ...
      <Dialog open={!!deletingProduct} onOpenChange={() => setDeletingProduct(null)}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              {`Are you sure you want to delete the product "${deletingProduct?.name || 'Unknown Product'}"?`}
