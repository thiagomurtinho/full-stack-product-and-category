...
export const productsApi = {
  ...
  getAll: async (params?: { 
    ...
    const [response, error] = await api.get<any>(`/products${queryParams}`)
  ...
  getById: async (id: string): Promise<ProductWithCategories> => {
    const [data, error] = await api.get<any>(`/products/${id}`)
  ...
  getWithCategories: async (id: string): Promise<ProductWithCategories> => {
    const [data, error] = await api.get<any>(`/products/${id}/with-categories`)
  ...
  getBySlug: async (slug: string): Promise<ProductWithCategories> => {
    const [data, error] = await api.get<any>(`/products/slug/${slug}`)
  ...
  create: async (product: CreateProduct): Promise<Product> => {
    ...
    const [data, error] = await api.post<any>('/products', validatedData)
  ...
  update: async (id: string, product: UpdateProduct): Promise<Product> => {
    ...
    const [data, error] = await api.put<any>(`/products/${id}`, validatedData)
  ...
  delete: async (id: string): Promise<void> => {
    const [_, error] = await api.delete(`/products/${id}`)
  ...
  getCount: async (): Promise<ProductCount> => {
    const [data, error] = await api.get<any>('/products/count')
